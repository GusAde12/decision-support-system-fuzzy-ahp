        ...
        <form method="post" action="aksi.php?act=rel_alternatif_ubah&ID=<?= $row->kode_alternatif ?>">
            ...
            foreach ($rows as $row) : ?>
                <div class="form-group">
                    ...
                    <input class="form-control" type="text"  name="ID-<?= $row->ID ?>" required value="<?= $row->nilai ?>" />
                </div>
            <?php endforeach ?>
            <button class="btn btn-primary"><span class="glyphicon glyphicon-save"></span> Simpan</button>
            <a class="btn btn-danger" href="rel_alternatif.php"><span class="glyphicon glyphicon-arrow-left"></span> Kembali</a>
        </form>
    </div>
</div>
</div>